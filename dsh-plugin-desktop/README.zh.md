...
本定制版 Evo 不会访问上游 DSH Desktop 发布服务。后台定时检查已在 `cordis.patch.yml` 中关闭，版本端点与安装包下载端点都被有意设置为不可解析，因此托盘中的 **Check for Updates…** 命令不会报告任何会替换当前构建的上游版本。更新以 `evo` 频道的 GitHub Release 形式发布并需手动安装，所以下面描述的下载流程在本定制版中不可达。开发运行、未打包启动与 Linux 不会下载安装包。
